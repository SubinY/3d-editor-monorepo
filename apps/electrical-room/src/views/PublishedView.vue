...
import type { CatalogItem, EditorSession, NodeInteractionEvent } from '@mh/3d-editor'
import {
...
onMounted(async () => {
  ...
  if (!sceneId || !Number.isInteger(version) || version <= 0) {
    bootError.value = '无效的发布版本'
    return
  }

  const bundle = await api.getPublish(sceneId, version)
  if (!bundle?.document || !bundle.assetPack) {
    bootError.value = `未找到发布包 v${version}`
    return
  }

  sceneName.value = bundle.name || bundle.document.name
  versionLabel.value = `v${bundle.version}`

  const catalog = createPackCatalog(bundle.assetPack as Record<string, CatalogItem>)
  ...
  const comm = await loadCommBundle()
  const configs =
    usedIds.length > 0 ? toDataSourceConfigs(comm, { sourceIds: usedIds }) : []
...
<template>
  <div class="preview">
    ...
    <aside class="panel">
      ...
      <template v-else>
        <p class="meta">{{ sceneName }} · {{ versionLabel }}</p>
      ...
      <button v-if="!bootError" @click="toggle">
        {{ running ? '暂停点位轮询' : '恢复点位轮询' }}
      </button>
