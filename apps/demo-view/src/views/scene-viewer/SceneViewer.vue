<template>
  <div class="viewer-page">
    <header class="viewer-header">
      <div class="header-left">
        ...
        <div class="app-brand">
          ...
          <span class="brand-text">场景查看</span>
        </div>
        <span v-if="sceneName" class="name-badge">
      ...
      <Transition name="info-slide">
        <aside v-if="selectedInfo" class="info-panel">
          ...
          <div class="info-body">
            ...
            <div v-if="selectedInfo.assetName" class="info-row">
              <span class="info-label">源资产</span>
              <span class="info-value accent">{{ selectedInfo.assetName }}</span>
            </div>
          </div>
        </aside>
      </Transition>
    ...
    <footer class="viewer-footer">
      ...
      <span v-if="floorInfo" class="footer-item">
        <Icon icon="mdi:floor-plan" class="footer-icon" />
        {{ floorInfo }}
      </span>
      <span class="footer-spacer" />
      <span class="footer-item hint">鼠标拖拽旋转 · 滚轮缩放 · 右键平移 · 点击查看对象信息</span>
    </footer>
  </div>
</template>
...
function handlePick(event: PointerEvent) {
  ...
  if (result.object && !result.object.userData?.nonSelectable) {
    ...
    selectedInfo.value = {
      ...
      assetName: target.userData?.assetName
    }
  } else {
    clearSelection()
  }
...
onMounted(async () => {
  if (!canvasRef.value || !sceneId) return
  ...
  const fWidth = custom?.floorSize?.width ?? 60
  const fDepth = custom?.floorSize?.depth ?? 30

  await ctx.applyPreset(factoryPreset, {
    floor: { width: fWidth, depth: fDepth, showFrame: true },
    ...DEFAULT_FACTORY_ENV,
    camera: { fov: 40, position: [0, 60, 100], target: [0, 0, 0] }
  })

  ctx.axisHelper.setEnabled(false)
  floorInfo.value = `${fWidth}m × ${fDepth}m`
...
.footer-icon { font-size: 13px; color: var(--color-accent); opacity: 0.6; }
